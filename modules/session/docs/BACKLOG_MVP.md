...
| 5 | [SESSION-33](MVP/SESSION-33_SCHEDULEDSTART_TRUE_INSTANT.md) | `Session.scheduledStart` becomes a true instant (`TIMESTAMPTZ`), with a creator-zone fallback for location-less sessions — foundation for SESSION-34/35 (location-timezone redesign) | `TODO` |
| 6 | [SESSION-34](MVP/SESSION-34_HISTORY_DATE_COUNTS_AT_TIME_ZONE.md) | Rewrite `findHistoryDateCounts` using `AT TIME ZONE`, retiring `zoneOffsetSeconds` — supersedes SESSION-31's reverted point-fix | `TODO` |
| 7 | [SESSION-35](MVP/SESSION-35_DISCOVER_CALLER_ZONE_FILTERS.md) | Rewrite `/discover`'s `date`/`startTime` filters for caller-zone semantics, retiring the `EXTRACT`+`MOD` correction — partially supersedes SESSION-25's timezone mechanics | `TODO` |

...
| 17 | SESSION-17 | Partial index on `session_outbox_events` scoped to `status = 'PENDING'` | `DONE` |
| 18 | SESSION-18 | Notify JOINED participants when a session transitions to ONGOING | `TODO` |
...
**Status:** `DONE` — see `modules/session/docs/SESSION-17_OUTBOX_PENDING_PARTIAL_INDEX.md`

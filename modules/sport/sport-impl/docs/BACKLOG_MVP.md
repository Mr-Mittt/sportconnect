...
| # | Ticket | Title | Status |
|---|---|---|---|
| 1 | [A10](MVP/A10_NO_DELETE_PATH_FOR_A_STORED_PROFILE_ATTRIBUTE.md) | No delete path for a stored profile attribute — merge-only semantics retained by A9, so a stored key can never be removed; stale keys also consume the 4KB cap. **Sharpened by v2:** `DEFINITION_LIST` gets a working clear path for free (an empty list), which makes its absence for `STRING`/`ENUM` user-visible. **Widened 2026-09-03:** also lazily prunes values orphaned by an admin physically deleting a definition — dropped from the merge on the profile's next write | `TODO` |

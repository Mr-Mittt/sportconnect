...
| # | Ticket | Title | Status |
|---|---|---|---|
| 1 | [A22](MVP/A22_CALLER_SCOPED_SPORT_PROFILE_READS.md) | Caller-scoped sport-profile reads (drop `{userId}`) — `GET /profiles/user/{userId}` → `GET /profiles` (empty list if none); `GET /profiles/user/{userId}/sport/{sportId}` → `GET /profiles/sport/{sportId}` (`404` if none); owner from the JWT principal, no ownership `403`. **Supersedes A20's owner-only list gate** (keeps its `?includeInactive` overload); `SecurityConfig` matchers reworked → `401` anonymous. Census: no cross-module backend consumer; client covered by A20's 2 tickets (widened to the path change) | `TODO` |

...
| # | Ticket | Title | Status |
|---|---|---|---|
| 1 | [A20](MVP/A20_ISRESUME_REACTIVATION_MODE.md) | `isResume` mode on sport-profile reactivation — new `Boolean isResume` on `CreateUserSportProfileRequest`; when set, re-adding a soft-deleted profile keeps its old scalar columns (null-check merge) and runs `attributes` through A10's prune + merge pipeline instead of wiping. Revises A7. **Depends on A10**; client resume/fresh chooser filed alongside | `TODO` |

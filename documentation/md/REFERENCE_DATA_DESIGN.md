...
**Delta (2026-09-25, REF-2 implemented):** as built, and where it differs from the text above —

- `geo/countries.tsv` ships **all** countries (239 rows, 237 distinct codes; user decision at pickup), so REF-4 is rows only.
  A polygon match for a country with no seeded/active row falls through to the next signal (never an error).
- Total bundle is ≈ 0.8 MB (countries 742 KB at 1:50m/50% kept, Vietnam provinces 68 KB from **1:10m** — the 1:50m admin-1 file
  has no Vietnam provinces — at 40% kept, timezones 7 KB), inside the 2 MB budget. Generated by the checked-in
  `reference-impl/geo-data/build-geo-data.mjs`; commands and licences are in `reference-impl/CLAUDE.md`.
- **`tz-country.tsv` is `zone1970.tab` plus `backward` link aliases**, not `zone1970.tab` alone: browsers report link names
  (`Asia/Saigon`) that `zone1970.tab` omits. Only single-country zones are kept, so merged zones deliberately resolve to no
  country (`Asia/Bangkok`, `Asia/Tokyo` = `JP,AU`, `Europe/Paris` = `FR,MC`); the caller falls through to the locale subtag.
- Country candidates are matched to seeded rows with **one** `IN` query (coordinates, timezone, then each locale region), the
  first seeded candidate wins; language falls back to the resolved country default language only when no locale matched and
  that language is active. `region` is returned only when coordinates won and only if it belongs to the resolved country.
- Validation is bean validation on the request (a violation is the existing `400` envelope); junk locales and unknown zone
  ids are ignored, not rejected. The both-or-neither rule reports under the field name `coordinatePairComplete`.
- Known data limits, both pinned in `GeoBoundaryResolverSpec`: coastal points can land offshore (Manhattan, Suva), and
  Natural Earth itself places central Cần Thơ in `VN-73` Hậu Giang (true of the unsimplified source too).

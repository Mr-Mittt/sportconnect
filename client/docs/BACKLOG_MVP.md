...
| 12 | [SPORT-12](MVP/SPORT-12_DEACTIVATE_REACTIVATE_UI_VISUAL_REGRESSION.md) | Visual regression for SPORT-10's deactivate/reactivate UI — **regenerate the 3 `profile-settings-*` baselines** (the Settings tab gained the Active toggle row) + new coverage for the Settings-tab inactive read-only state, `SportProfileStatusConfirmDialog`, `ReactivateSportNudgeDialog`, muted `SportSwitcher` pills. Follow-up from SPORT-10 (pattern: CLIENT-NOTIF-2 / CLIENT-SESSION-12 / GRP-10 + the HF-1x regen tickets) | `TODO` |

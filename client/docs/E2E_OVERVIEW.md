...
| State | Setup | Expects |
|---|---|---|
| `not-joined` | `mockDiscoverableSession` ("Weekend 5-a-side"), View details | "Join" button visible. **CLIENT-SESSION-17:** the fixture now carries `attributes` (two `#ref` values + one own node), so this state also frames the read-only "Session detail" summary — its 3 baselines change and need an `update-baselines` dispatch |
...
fold. CLIENT-SESSION-17 adds the `session-detail-ref` state, which deliberately scrolls the
"Session detail" section into view — its 3 baselines (`create-session-session-detail-ref-{375,768,1280}.png`)
are new and must be generated by an `update-baselines` dispatch; the other 9 stay byte-identical.**

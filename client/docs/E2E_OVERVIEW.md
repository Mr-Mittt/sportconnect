...
`/matches`, rewriting `home-feed-journey.spec.ts` step 6), `CLIENT-SESSION-22_DISCOVER_SEARCH_FILTER_UI.md`
(wires Discover's Date/Location/Time filter pills to the real `GET /sessions/discover`/`GET
...
`GET /sessions/discover` page for real), `CLIENT-SESSION-27_ESCAPE_CLOSES_WHOLE_MODAL_INSTEAD_OF_JUST_THE_POPOVER.md`
(adds one `home-feed-journey.spec.ts` test — Escape closes only the open Discover filter popover, not the modal),
`CLIENT-SESSION-32_DISCOVER_MODAL_LOCATION_POPOVER_CLIPPED_AT_375PX.md` (adds one `home-feed-journey.spec.ts` test, run at
375px and 320px — every Discover-modal filter popover stays inside the dialog's box).
...
### `e2e/flows/home-feed-journey.spec.ts` (HF-11, one main `test()` with 8 steps, plus one separate `test()`)
...
**Separate tests — Time/Location popover regressions, CLIENT-SESSION-22 Delta 2 + CLIENT-SESSION-29:**

| Test | What it checks | Notes |
|---|---|---|
| `the "Join a match" modal's Time filter popover is actually clickable` | Seeds empty upcoming matches (`seedEmptyUpcomingMatchesOnNextLoad`, new `sessionsEmpty` override) → "Join a match" → `SessionDiscoverModal` opens with a real discoverable session card rendered → opens the Time filter, clicks "Before", asserts `aria-pressed="true"` and the trigger label updates | Found live: a *modal* `Dialog` sets `pointer-events: none` on `<body>`, restoring `auto` only on its own Content — `Popover`'s portaled content is a sibling of that Content, not a descendant, so clicks passed through to the session card underneath. Fixed at `shared/ui/popover.tsx` (`pointer-events-auto`). jsdom can't reproduce this class of bug (couldn't even get the popover to report `open` reliably nested in a real Dialog) — real-browser-only regression coverage. Two related findings from the same investigation, filed as **CLIENT-SESSION-27** (Escape closes the whole Dialog too — still open) and **CLIENT-SESSION-28** (the Location filter's search input can't be typed into in this modal — fixed below, CLIENT-SESSION-29) |
| `the "Join a match" modal's Time/Location popovers stay focused and typeable` | Selects the Badminton pill → "Join a match" → opens the Time filter, clicks Before, clicks the Hour input, asserts it stays focused, fills `05`, blurs, asserts the value stuck and the trigger label updated → closes it, opens the Location filter, asserts the search input stays focused and a typed value sticks | **CLIENT-SESSION-29:** root-caused both this ticket's own "Time filter doesn't auto-apply on hour/minute edit" bug and CLIENT-SESSION-28's search-input bug to the exact same cause — Dialog's `FocusScope` trap treats a `Popover`'s `document.body`-portaled content as "escaped the boundary" (`container.contains(target)` is `false`) and yanks focus back on every `focusin`. Fixed at the shared primitive: new `shared/ui/floatingPortalContainer.ts` context — `DialogContent` provides its own Content DOM node, `PopoverContent` portals into it (`Popover.Portal`'s own `container` prop) instead of `document.body` whenever available, so the popover becomes a real DOM descendant and the trap never fires. Confirmed live before writing this test that both symptoms reproduced (`document.activeElement` went `null` on click) and that the fix resolves both, with no popover clipping/mispositioning despite the Dialog's `overflow-hidden`/`transform` (Radix Popper's `strategy: 'fixed'` accounts for a transformed containing block). CLIENT-SESSION-27's Escape bug is untouched — confirmed it's a different Radix subsystem (`DismissableLayer`'s mount-order layer stack, not `FocusScope`'s DOM-containment check) |
| `tabbing out of the Time filter's Hour input keeps the popover open with results on screen` | Routes `/sessions/discover` with `startTime`/`startTimeFilter` stripped so results stay **non-empty** → "Join a match" → Time → Before → fills Hour `05` → **Tab** → asserts the Minute input is focused, the Hour value stuck, and the trigger reads `Before 05:MM` | **CLIENT-SESSION-29 (2026-09-24), found by CI + a real-browser repro after several wrong theories.** During Tab `document.activeElement` is briefly `<body>`; the Hour commit changes the discover filters and (no `placeholderData`) synchronously removes the non-empty card grid inside that window, so the Dialog's `FocusScope` focuses its own container, which the nested Popover read as focus outside itself and dismissed. Fixed in `DiscoverTimeFilter` (parent update wrapped in `startTransition`) and shared `PopoverContent` (ignores focus landing on the Dialog's own container). Every earlier local run passed because the mock's own time filter *empties* the list at that moment, leaving nothing to remove — this test forces the condition. Verified to **fail without the fix** |
...
### `e2e/flows/matches-journey.spec.ts` (CLIENT-SESSION-1/…/CLIENT-SESSION-9, CLIENT-SESSION-17, CLIENT-SESSION-21, CLIENT-SESSION-22, CLIENT-SESSION-23, CLIENT-SESSION-29, CLIENT-SESSION-30, SPORT-10, one `test()` with 11 steps + steps 3b/3c/5b/5d/8c/10b/10c/10c-2 + 3 separate `test()`s)
...
authenticated identity to actually request-join as). **CLIENT-SESSION-29:** `mockRequestedSession`
("Wednesday scrimmage", group-linked, pre-seeded via `mockUserRequestedRow` — the test user's own
`REQUESTED` row, same fixture `home-feed-journey.spec.ts`'s CLIENT-SESSION-12 coverage already
seeded) is now also asserted here (step 10c), the first `matches-journey.spec.ts` use of it. All
session fixtures carry
...
steps 9-11 (plus 10b) are what's actually new. **CLIENT-SESSION-22:** `mocks/handlers/sessions.ts`
gained a shared `discoverableSessions()` helper (filters by `sportId`/`title`/`locationId`, reused
by both `GET /sessions/discover` and the new `GET /sessions/discover/counts`) — `date` itself is
still not filtered by this mock (never was; every fixture session carries a fixed `scheduledStart`
...
blocks clicks behind it — a spec that joins/accepts must dismiss it ("Got it", or "Open session" when the join came from a card, which also opens that session's detail). `mocks/handlers/sessions.ts`'s `DELETE
/sessions/:id/leave` now accepts `JOINED`/`INVITED`/`REQUESTED` rows (real SESSION-9 contract; it used to 400 on all but
`JOINED`, so "Cancel my request" couldn't be exercised).
...
| 5b. card-level Accept + Leave | Click `mockInvitedSession`'s ("Tuesday drop-in") card "Accept" → flips to "Leave"; click `mockGroupSession`'s card "Leave" → **its card disappears** from Upcoming | CLIENT-SESSION-9. **CLIENT-SESSION-23:** `/upcoming` is participant-scoped, so leaving removes the card (the old group fan-out kept it with a "Join" button). **CLIENT-SESSION-30:** Accept resolves to JOINED, so the "You joined the session" pop-up opens over the page and offers "Got it" and, because it came from a card, "Open session" — the step clicks it, and that session's detail (`Tuesday drop-in`, with "Leave") opens and is closed before the "Leave" card assertion. No detail dialog opens for either click — proves the card's own action button round-trips through `sessionKeys.all` invalidation |
...
| 8c. Status/Open slots/Fee filters narrow Discover (CLIENT-SESSION-29) | Check Status "Preparing" (never true for the mock's SCHEDULED-only baseline) → empty → uncheck → `mockDiscoverableSession` back. Fill the Open slots input with 7 (excludes it, 6 open < 7) → empty → 6 (6 open >= 6) → visible → Clear. Check Fee "Split cost" (it's FREE) → empty → uncheck → visible again | **Must run before step 9** — step 9 joins `mockDiscoverableSession`, which then drops out of the discoverable pool entirely (already-participating exclusion), so this step would have nothing left to assert against if it ran after. `mocks/handlers/sessions.ts`'s `discoverableSessions()` extended to actually filter on `status`/`minOpenSlots`/`feeType`/`maxFeeAmountVnd` (previously title/locationId only) so these assertions prove something real. **Revision (2026-09-23):** Open slots is now a direct inline input (`getByLabel('Minimum open slots')`), no Popover trigger to click first; Status became single-select (mutually exclusive, same "click again to clear" toggle) but the 2-click check/uncheck sequence this step already used still round-trips the same way |
| 9. discover → join → moves to My sessions | `mockDiscoverableSession` visible in Discover, not in My sessions → open its detail → **CLIENT-SESSION-17: the read-only "Session detail" `region` shows the stored `#ref` values** (`LIST` `#ref` `match/racketModel` as chips "Yonex Astrox 99" / "Li-Ning Axforce 90", `SINGLE` `#ref` `match/racketBrand` as plain "Yonex", own node `match/format` as "Doubles") → Join → **CLIENT-SESSION-30: the "You joined the session" pop-up opens on top of the detail — no "Open session" button, the join came from the detail — and is dismissed with "Got it"** → Leave button appears → close → now absent from Discover, present in My sessions | Both `useDiscoverFilters`/`useJoinedSessions` invalidate off the same `sessionKeys.all` root, so no manual reload/refetch is needed; the mock's `GET /sessions/discover` handler excludes any session the caller currently has a `JOINED`, `REQUESTED`, or `INVITED` row for (widened from `JOINED`-only by SESSION-42, matching the real backend's own widened exclusion — `mockDiscoverableSession` has no pre-seeded participant row for the test user, so this step's own assertions are unaffected by the widening). The `#ref` assertions prove `SessionAttributesSummary` maps a `#ref` node's stored shape onto the right render type by `cardinality`, not its inherited scalar `type` — `mockDiscoverableSession` now carries `attributes` |
...
| 10c-2. pending request hint + re-request pop-up (CLIENT-SESSION-30) | From the Requested section open "Wednesday scrimmage" → detail shows "Need host approval. Created by …" and "Waiting for host approval." → Cancel → "Join" returns and the hint is gone → Join → the "Join request sent" pop-up ("Waiting for host approval. Feel free to chat while you wait.", no "Open session") → "Got it" → the detail is back to "Cancel" + hint → Close | `mockRequestedSession` is `autoApprove: false`, so the re-request lands on `REQUESTED` (not `JOINED`); ends in the same state it started, so step 11 is unaffected. Relies on the leave handler accepting a `REQUESTED` row |
...
| `requested` | `mockRequestedSession` ("Wednesday scrimmage", **new fixture** — mockUser's own pre-seeded `REQUESTED` row), View details | "Cancel" button visible. **CLIENT-SESSION-30:** "Waiting for host approval." now sits to its left, and every state shows the approval-mode label + 24h time — all 24 baselines change, regenerate via `update-baselines` |
...
date-popover state; popovers are opened by click (Escape dismissal inside the modal — CLIENT-SESSION-27 — is covered by
`home-feed-journey.spec.ts`, not needed for screenshots).

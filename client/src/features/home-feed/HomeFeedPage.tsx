...
export function HomeFeedPage() {
  ...
  // CLIENT-SESSION-29 revision (2026-09-23) — the modal now pre-fills its own sport dropdown from
  // this seed, falling back to the caller's first held sport when the pill is 'all' (no single
  // sport to seed from).
    ...
    <ModalAnchorProvider value={modalAnchorBottom}>
      <main className="py-4">
        ...
        <SessionDiscoverModal
          ...
          onStartTimeChange={discoverModalData.setStartTime}
          onClearTimeFilter={discoverModalData.clearTimeFilter}
          sessions={discoverModalData.sessions}
          isLoading={discoverModalData.isLoading}
          isError={discoverModalData.isError}
          hasMore={discoverModalData.hasMore}
          isFetchingMore={discoverModalData.isFetchingMore}
          onLoadMore={discoverModalData.onLoadMore}

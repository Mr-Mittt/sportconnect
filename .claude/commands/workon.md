...
3. **Run the `e2e` Playwright project (`pnpm e2e`) — always, on every client ticket, not only when the ticket "has E2E scope".** A ticket's Tests section scopes the *new* tests it adds; it never waives the *existing* e2e suite. This is the client counterpart of the backend's unconditional `:server:test` — a green Vitest run alone is **not** sufficient to call Phase 5 done, because RTL and Playwright disagree on defaults (RTL `getByLabelText`/`getByText` match **exact**; Playwright `getByLabel`/`getByText`/`getByRole({name})` match **substring + normalized whitespace**; RTL can query hidden/`forceMount`ed nodes, Playwright's `toBeVisible` and role queries exclude them). Runs headless locally (`playwright.config.ts` `headless: true`); pass `--headed`/`--ui` only for an actual debugging session.
   - **Before running, `grep client/e2e/` for every component/page/hook in the diff** — anything that returns a spec file is a browser-covered surface you have changed and must re-verify:
     ```bash
     git diff --name-only | grep -oE '[A-Za-z][A-Za-z0-9]*\.(tsx?|ts)' | sed 's/\.[^.]*$//' | sort -u \
       | while read n; do grep -rl "$n" client/e2e/flows client/e2e/visual 2>/dev/null; done | sort -u
     ```
   - **Also grep `client/e2e/` for any label / button text / `aria-label` / role name this ticket adds or renames.** A new name that *contains* an existing one (e.g. adding `"Session schema document (JSON)"` next to `"Schema document (JSON)"`) passes every Vitest test and trips the old Playwright locator's strict-mode on two matches. Fix in the same change: non-overlapping names, or `{ exact: true }` on the existing e2e locator.
   - **A pre-existing flake is not a failure and a green isolated re-run is not a pass for a real break.** On any failure, re-run that spec in isolation to separate a suite-parallelism flake from a real regression, and state which in the Phase 6 summary.
   - **Never record an e2e result you did not run.** If the `e2e` project genuinely cannot run in this environment, the summary says "e2e not run — <reason>", the same discipline the "Visual-regression expectation" line forces — never "e2e stays green" as an assertion.
   - **If any spec file was added, removed, or had a test added/removed/materially changed, update `client/docs/E2E_OVERVIEW.md`'s catalog before calling this ticket done** — the living reference for every e2e test case; silently rotting out of date defeats its purpose. This includes the directory listing (§3) and the per-file test table (§6); cross-reference the ticket's own summary doc from the "Related docs" line if it's the origin of a new spec file.
4. **Run the `visual-regression` Playwright project** if the ticket touches any baselined visual surface (or added a `visual-regression` spec).
...
5. Start the Vite dev server and walk the ticket's happy path in a browser
6. If the hook hits a real backend endpoint, verify once against the actually running backend (`./gradlew :server:bootRun`) — MSW passing is not proof the real contract matches
...
2. Write an implementation summary covering the Phase 3 design (the approved plan, restated — not just a link back to chat), what was built, key decisions, and non-obvious constraints. If what was built diverged from the approved design (e.g. a test premise broke, an edge case forced a different approach), say so explicitly rather than silently updating the design to match the outcome.
   - **Client tickets: the summary MUST carry an "E2E" line** — the actual result of the Phase 5 `pnpm e2e` run: "`e2e` project N passed" (and, if any spec failed, which specs, whether each is a real regression or a pre-existing parallel-load flake, and the isolated-re-run proof). If the `e2e` project genuinely could not run, say "e2e not run — <reason>"; never write "e2e stays green" as an unverified assertion.

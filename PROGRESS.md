...
- **C4 (2026-09-03):** Map swallowed Spring MVC exceptions to their real HTTP status — `GlobalExceptionHandler` now `extends ResponseEntityExceptionHandler`, so a request that can't be dispatched/bound no longer collapses to the `@ExceptionHandler(Exception.class)` catch-all's generic 500: no route → **404**, wrong method → **405**, unsupported/absent `Content-Type` → **415**, unacceptable `Accept` → **406**, unreadable JSON body → **400**, un-bindable `@PathVariable` type → **400** (`MissingPathVariableException` stays 500 — a genuine mapping bug — but now `ApiResponse`-wrapped + logged). A single `handleExceptionInternal` override swaps Spring's `ProblemDetail` body for `ApiResponse.error(<generic per-status message>)` (never `ex.getMessage()` — could leak the path/header/parser detail); a body a specific override already built (the field-map `MethodArgumentNotValidException`, the `"<name> is required"` `MissingServletRequestParameterException` — both re-expressed as `@Override`s of the base `protected` methods rather than `@ExceptionHandler` re-declarations, which would throw "Ambiguous @ExceptionHandler" at startup) is passed through untouched. The 5 business-exception handlers and the `Exception`→500 fallback are unchanged. **Widened at pickup** from the filed "`NoResourceFoundException` → 404 only" (user decision) — C1 had already had to hand-patch two of these back one at a time. Found by sport A22's Phase 5 smoke test. 4 new `GlobalExceptionHandlerSpec` cases + new `GlobalExceptionMappingIntegrationTest` (5 cases through the real pipeline). Green: `:modules:common:test`, full `:server:test`, live smoke. See `modules/common/docs/MVP/C4_NO_RESOURCE_FOUND_MAPS_TO_404.md`.
- **MVP backlog:** 4 tickets (C1–C4) in `modules/common/docs/BACKLOG_MVP.md`, all **`DONE`**
...
- **Attribute framework extraction (planned 2026-09-08,
  ...
  multi-select UI). **C5–C9 `DONE` 2026-09-08** (all pure addition; 293 common tests green — the
  common half of the split is complete, sport A23 unblocked). C5: ADR + the DTO tree
  (`com.sportconnect.common.attributes`, sealed `AttributeNode`/`AttributeField`, flat
  `ResolvedAttribute*`, `AttributeJson` strict mapper). C6: single-schema `validate/`
  ...
  C9: `pair/` — `DerivedSchemaValidator` (reuses C6's own-node/definitions/leaf checks — those
  three helpers widened to `public`; D9 `#ref` contract: required explicit `key` + `cardinality`,
  globally-unique `#ref` resolving to a live base node, dangling → strict reject),
  `DerivedSchemaExpander` (lenient-drop a stale `#ref`, clone the base target's own subtype
  re-keyed, pull the base `definitions` closure, return `Map<path, RefExpansion{basePath,
...
- **SESSION-37 (`DONE`, 2026-09-21,
  ...
  design). **Final scope, built:** new `Session.isPublic` column (`V071`, standalone → `true`,
  group → `false`, replaces `groupId IS NULL` as Discover's predicate — not exposed on
  `SessionResponse`, user decision); redesigned partial index (`idx_sessions_sport_id_standalone`'s
  predicate becomes `is_public = true`, same columns — a second index the user proposed was checked
  against real `SessionGenerationJob` consumers and dropped as unnecessary); `/discover` reverts to
  a **single required** `date` (not the intermediate `List<LocalDate>` design) with a smart
  today/future floor (a past date silently clamps to today's `now()` floor), `startTimeFilter`/
  `startTime` independently optional (`startTime` alone defaults `AFTER_OR_EQUAL`; unpaired
  `startTimeFilter` silently ignored), page size default `20`→`10`; `status` drops `ONGOING` from
  the default (an explicit `ONGOING` is stripped, not a 400 — falls back to the default only if
  stripping empties the list). **The "browse many days" job that motivated this ticket's earlier
  redesigns moved to a new, separate ticket — SESSION-39** (`SessionCount`, a `List<LocalDate>`
  ...
  scoping this ticket, unrelated to its actual scope, filed separately), a delta note on client
  **CLIENT-SESSION-25** (SESSION-39's counts could enhance its date picker, not required for it),
  and a new client follow-up **CLIENT-SESSION-26** (Discover's page-size drop from 20→10 exposed
  that the Discover grid has no "load more" pagination UI at all — consumer census finding at
  pickup, filed rather than left unbuilt). Fixed 5 other IT fixture files that build `Session`
  directly, bypassing `createSession`, and so needed an explicit `.isPublic(...)` for the new
  `NOT NULL` column — found only by a full `:server:test` run, not any mocked unit test. Green:
  `session-impl` (full Spock suite) + `:server:test` (full suite, 0 failures) + live verification
...
### Reference Data — Language / Country / Region + client i18n (designed 2026-09-25; **REF-1, REF-2 implemented**)
- **REF-1 DONE (2026-09-25):** `reference` module (`languages`/`countries`/`regions`, V072 + V073 seed: `en`+`vi`, **Vietnam only** country, 63 Vietnam regions) + public `GET /api/reference/{languages,countries,countries/{id}/regions}`; a nullable per-country **default language** (`countries.default_language_code`, Vietnam → `vi`, V074; browser language list still wins, REF-2/CLIENT-REF-1 apply it); 21 Spock + `ReferenceApiIntegrationTest` (14), `:server:test` 317 green, verified on real Postgres. Countries narrowed to Vietnam at pickup → **REF-4** seeds the rest. Summary: `modules/reference/docs/MVP/REF-1_REFERENCE_MODULE_TABLES_SEEDS_AND_PUBLIC_READS.md`
- **REF-2 DONE (2026-09-25):** offline `GeoBoundaryResolver` (JTS `STRtree`, lazy double-checked field like LOC-4) over bundled Natural Earth polygons (**all countries** 1:50m + Vietnam's 63 provinces 1:10m) and an IANA timezone table (`zone1970.tab` + `backward` aliases; merged zones deliberately resolve to no country) — 0.8 MB, regenerated by `reference-impl/geo-data/build-geo-data.mjs`; public `POST /api/reference/resolve` (coordinates > timezone > locale, one exact `SecurityConfig` permit) + `ReferenceService.resolveByCoordinates` for LOC-5; results are seeded **active** rows only (a Paris coordinate is an all-null `200` until REF-4). 88 Spock (real polygons) + `ReferenceApiIntegrationTest` 26 (permit mutation-checked), `:server:test` 329 green, verified on real Postgres. Summary: `modules/reference/docs/MVP/REF-2_BOUNDARY_RESOLVER_AND_RESOLVE_ENDPOINT.md`
...
- New `reference` domain: `languages` / `countries` (**Vietnam only for now** — REF-4 seeds the rest) / `regions` (**Vietnam only** — deliberate, see design § 8) + public `GET /api/reference/**`
...
- Tickets filed: `reference` **REF-1..4**, `user` **U16**, `location` **LOC-5**, `sport` **A24**, client **CLIENT-I18N-1** (supersedes V1 `I18N-1`), **CLIENT-REF-1..3**, **CLIENT-I18N-2**
...
| STOMP relay dev host port | `infra/docker-compose.dev.yml` publishes RabbitMQ's STOMP port as `8613:61613` (host `8613`, not the STOMP-standard `61613`); `application-dev.yml` sets `app.stomp-relay.port: 8613`. Reason: on Windows, Hyper-V/WSL *dynamic* port reservations (range `49152–65535`, shift on reboot) can grab `61613` so Docker silently fails to bind the host side — `docker ps` still prints the requested mapping, which is the misleading part. A host port below 49152 makes the conflict structurally impossible. Container-internal port and the `application.yml`/prod default stay `61613`; CI unaffected (Linux). Recreating the container, the STOMP plugin, and the `:::61613` IPv6 bind are all red herrings — see `server/README.md` Troubleshooting |
